...
- **内建材料数据库**  
  ...
  - 一键选择材料自动填入 $V_0$、$\alpha_L$、$\theta_D$、$\gamma$、$B$ 等
...
- **常数热膨胀系数外推 $V_m(T)$**  
  - 使用：$V_m(T) \approx V_0 \bigl[1 + \alpha_V (T-0)\bigr]$，其中 $\alpha_V \approx 3\alpha_L$  
...
- 若取 $\alpha_L \in [20, 30]\times 10^{-6}\ \text{K}^{-1}$，可得到对应的体积区间，用于误差估计。
